...
# Module 1 – Data Warehouse – EX01 customers table
#
# sternero – 42 Málaga – Octubre 2026
#
# Asistente opcional (NO sustituye customers_table.*)
# - Comprueba PostgreSQL / tablas data_202*
# - Ejecuta customers_table.py o aplica customers_table.sql
# - Muestra COUNT(*) de customers y de cada origen
# ============================================================
...
PY_SCRIPT="$SCRIPT_DIR/customers_table.py"
SQL_SCRIPT="$SCRIPT_DIR/customers_table.sql"
...
print_header()
{
    ...
    echo -e "${CYAN}${BOLD}║  Module 1 – Data Warehouse – EX01 customers table          ║${RESET}"
...
list_data_tables()
{
    section "📚  Tablas data_202*"
    if ! check_container >/dev/null 2>&1; then
        docker ps --format '{{.Names}}' 2>/dev/null | grep -qx "$CONTAINER_NAME" || return 1
    fi
    psql_c "SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename LIKE 'data_202%' ORDER BY 1;"
    echo
}

run_python()
{
    section "🐍  customers_table.py"
    if [[ ! -f "$PY_SCRIPT" ]]; then
        err "Falta $PY_SCRIPT"
        return 1
    fi
    chmod +x "$PY_SCRIPT" 2>/dev/null || true
    info "Descubre data_202% y crea customers con UNION ALL"
    if ! ask_yes_no "¿Ejecutar ahora? (puede tardar)" "s"; then
        warn "Omitido"
        return 0
    fi
...
run_sql()
{
    ...
    info "Lista fija de meses (incluye data_2023_feb si existe)"
    warn "Si falta alguna tabla del SQL, fallará — usa Python o carga el mes"
    if ! ask_yes_no "¿Aplicar SQL vía docker cp + psql -f?" "s"; then
        warn "Omitido"
        return 0
    fi
    docker cp "$SQL_SCRIPT" "$CONTAINER_NAME:/tmp/customers_table.sql" 2>/dev/null || true
    docker exec -i "$CONTAINER_NAME" \
...
verify_counts()
{
    section "🔢  Verificación COUNT"
    if ! docker ps --format '{{.Names}}' | grep -qx "$CONTAINER_NAME"; then
        err "Sin contenedor"
        return 1
    fi
    echo -e "${BOLD}customers:${RESET}"
    psql_c "SELECT COUNT(*) AS customers_rows FROM customers;" 2>/dev/null \
        || { warn "¿Existe la tabla customers?"; return 1; }
    echo
    echo -e "${BOLD}Orígenes data_202*:${RESET}"
    psql_c "
SELECT tablename, (xpath('/row/c/text()', query_to_xml(format('SELECT COUNT(*) AS c FROM %I', tablename), false, true, '')))[1]::text AS n
FROM pg_tables
WHERE schemaname='public' AND tablename LIKE 'data_202%'
ORDER BY 1;
" 2>/dev/null || psql_c "
SELECT 'data_2022_oct' AS t, COUNT(*) FROM data_2022_oct
UNION ALL SELECT 'data_2022_nov', COUNT(*) FROM data_2022_nov
UNION ALL SELECT 'data_2022_dec', COUNT(*) FROM data_2022_dec
UNION ALL SELECT 'data_2023_jan', COUNT(*) FROM data_2023_jan;
"
    echo
    info "customers debe ser la SUMA de los orígenes (UNION ALL, sin deduplicar)"
}
...
start_pgadmin()
{
    ...
    local pgadmin_dir="$(dirname "$venv")"
    ...
    (
        ...
        nohup pgadmin4 >/tmp/pgadmin4_m1_ex01.log 2>&1 &
        echo $! >/tmp/pgadmin4_m1_ex01.pid
    )
...
start_services()
{
    ...
    echo
    info "En pgAdmin: Register → Server…"
    echo "  Host: localhost | Port: 5432 | Database: $(db_name) | User: $(db_user)"
    echo "  URL pgAdmin: http://127.0.0.1:5050"
}

show_menu()
{
    echo
    echo -e "${CYAN}${BOLD}  MENÚ EX01 – customers${RESET}"
    echo
    echo -e "  ${BOLD}1)${RESET}  Arrancar contenedor + pgAdmin ${WHITE}(entorno completo)${RESET}"
    echo -e "  ${BOLD}2)${RESET}  Comprobar contenedor"
    echo -e "  ${BOLD}3)${RESET}  Listar tablas data_202*"
    echo -e "  ${BOLD}4)${RESET}  Ejecutar customers_table.py ${WHITE}(recomendado)${RESET}"
    echo -e "  ${BOLD}5)${RESET}  Aplicar customers_table.sql"
    echo -e "  ${BOLD}6)${RESET}  Verificar COUNT(*)"
...
{
    print_header
    echo -e "Subject: unir todos los ${BOLD}data_202*${RESET} en la tabla ${BOLD}customers${RESET}."
    echo -e "Este menú es ayuda; la entrega es ${BOLD}customers_table.*${RESET}."
    echo
    local choice
    while true; do
        show_menu
        ...
        case "$choice" in
            ...
            3) list_data_tables; pause ;;
            4) run_python; pause ;;
            5) run_sql; pause ;;
            6) verify_counts; pause ;;
            7) open_psql; pause ;;
            q|Q) echo -e "${GREEN}Listo para EX02 (remove duplicates).${RESET}"; exit 0 ;;
