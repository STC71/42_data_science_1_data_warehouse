...
# - Ejecuta fusion.py o aplica fusion.sql
# - Muestra COUNT y columnas
# ============================================================
...
PY_SCRIPT="$SCRIPT_DIR/fusion.py"
SQL_SCRIPT="$SCRIPT_DIR/fusion.sql"
...
print_header()
{
    ...
    echo -e "${CYAN}${BOLD}║  Module 1 – Data Warehouse – EX03 fusion                   ║${RESET}"
...
count_tables()
{
    section "🔢  COUNT customers e items"
    if ! docker ps --format '{{.Names}}' 2>/dev/null | grep -qx "$CONTAINER_NAME"; then
        err "Sin contenedor"
        return 1
    fi
    info "SELECT COUNT(*) FROM customers;  /  FROM items;"
    psql_c "SELECT COUNT(*) AS customers_rows FROM customers;" 2>/dev/null \
        || warn "¿Existe customers? (EX01/EX02)"
    psql_c "SELECT COUNT(*) AS items_rows FROM items;" 2>/dev/null \
        || warn "¿Existe items? (Module 0 EX04)"
    echo
}

describe_customers()
{
    section "📋  Estructura de customers"
    if ! docker ps --format '{{.Names}}' 2>/dev/null | grep -qx "$CONTAINER_NAME"; then
        err "Sin contenedor"
        return 1
    fi
    info "Comando: \\d customers"
    docker exec -i "$CONTAINER_NAME" \
        psql -U "$(db_user)" -d "$(db_name)" -c '\d customers' 2>/dev/null \
        || warn "No se pudo describir customers"
    echo
    info "Tras la fusión deberías ver category_id, category_code, brand"
}
...
run_python()
{
    ...
    info "LEFT JOIN customers → items (product_id), sin perder eventos"
    warn "Puede tardar varios minutos (~19M filas)"
    if ! ask_yes_no "¿Ejecutar fusion.py ahora?" "s"; then
        warn "Omitido"
        return 0
    fi
...
run_sql()
{
    ...
    info "Misma lógica que el .py"
    warn "Puede tardar varios minutos"
    if ! ask_yes_no "¿Aplicar SQL vía docker cp + psql -f?" "s"; then
        warn "Omitido"
        return 0
    fi
    docker cp "$SQL_SCRIPT" "$CONTAINER_NAME:/tmp/fusion.sql" 2>/dev/null || true
    docker exec -i "$CONTAINER_NAME" \
        psql -U "$(db_user)" -d "$(db_name)" -f /tmp/fusion.sql
    echo
    info "Usa la opción de estructura / COUNT para verificar"
}
...
start_pgadmin()
{
    ...
    if check_pgadmin; then
        ok "pgAdmin ya estaba en marcha → http://127.0.0.1:5050"
        return 0
    fi
    ...
    if [[ ! -f "$cfg/config_local.py" ]]; then
        printf "DATA_DIR = '%s'\nSQLITE_PATH = '%s/pgadmin4.db'\n" "$data_dir" "$data_dir" > "$cfg/config_local.py"
    ...
    (
        ...
        nohup pgadmin4 >/tmp/pgadmin4_m1_ex03.log 2>&1 &
        echo $! >/tmp/pgadmin4_m1_ex03.pid
    )
    local attempt
    ...
    warn "pgAdmin no responde aún; revisa /tmp/pgadmin4_m1_ex03.log"
    return 1
}

start_services()
{
    section "🚀  Arrancar entorno"
    if [[ -z "$MODULE0_DIR" || ! -f "$MODULE0_DIR/ex00/docker-compose.yml" ]]; then
        err "No se encontró compose de Module 0"
        return 1
    fi
    if command -v docker-compose >/dev/null 2>&1; then
        ...
        err "Sin docker-compose"
        return 1
    fi
    sleep 2
    if docker ps --format '{{.Names}}' 2>/dev/null | grep -qx "$CONTAINER_NAME"; then
        ok "PostgreSQL en localhost:5432"
    else
        err "Contenedor no Up"
        return 1
    fi
    start_pgadmin || warn "PostgreSQL OK; pgAdmin requiere atención"
}
...
show_guides()
{
    ...
    echo
    info "Subject: fusionar customers + items sin perder información (LEFT JOIN)"
}

show_menu()
{
    echo
    echo -e "${CYAN}${BOLD}  MENÚ EX03 – fusion${RESET}"
    echo
    echo -e "  ${BOLD}1)${RESET}  Arrancar contenedor + pgAdmin"
    echo -e "  ${BOLD}2)${RESET}  Comprobar contenedor"
    echo -e "  ${BOLD}3)${RESET}  COUNT(*) customers e items"
    echo -e "  ${BOLD}4)${RESET}  Ejecutar fusion.py ${WHITE}(recomendado)${RESET}"
    echo -e "  ${BOLD}5)${RESET}  Aplicar fusion.sql"
    echo -e "  ${BOLD}6)${RESET}  Ver estructura de customers (\\d)"
    echo -e "  ${BOLD}7)${RESET}  Abrir psql"
    echo -e "  ${BOLD}8)${RESET}  Rutas de guías"
...
{
    print_header
    echo -e "Subject: fusionar ${BOLD}customers${RESET} e ${BOLD}items${RESET} sin perder información."
    echo -e "Entrega: ${BOLD}fusion.*${RESET}  ·  Este menú es solo ayuda."
    echo
    local choice
    while true; do
        show_menu
        ...
        case "$choice" in
            ...
            3) count_tables; pause ;;
            4) run_python; pause ;;
            5) run_sql; pause ;;
            6) describe_customers; pause ;;
            7) open_psql; pause ;;
            8) show_guides; pause ;;
            q|Q) echo -e "${GREEN}Module 1 – Data Warehouse: EX00–EX03 cubiertos por el subject.${RESET}"; exit 0 ;;
